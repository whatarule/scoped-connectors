...
外部サービス(Slack、Google Drive)へ範囲限定でアクセスするためのプラグイン集。
アクセス範囲を「サービス側の scope」と「プラグイン側の許可リスト」の二段で絞ることを設計原則とする。

書き込みは Slack の参加済みチャンネル（public / private）への投稿のみ。DM には投稿しない。
それ以外は読み取り専用とする。
書き込みを伴う操作は、実行前に人が内容を確認する段階を必ず挟む。
確認と実行は別の呼び出しになるため、確認した内容と実行する内容が同じであることを
実装で照合する(確認を挟むだけでは、内容が入れ替わったときに承認が効かないため)。
